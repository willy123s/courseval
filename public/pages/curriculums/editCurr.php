<div class="flex flex-row justify-between items-center mb-6">
    <p class="font-semibold text-lg text-slate-500">Create Curriculum</p>
...
<div class="">
    <form action="/curriculums/update" method="post">
        <input type="hidden" name="csrf_token" id="csrf_token" value="<?= $_SESSION['csrf_token'] ?>">
        <input type="hidden" name="id" id="id" value="<?= $curr->getId() ?>">

        <div class="relative mb-6">
            <input type="text" name="currname" id="currname" class="px-4 py-3 peer w-full border focus:outline-none focus:ring-2 focus:border-brand focus:ring-brand/20 rounded-md border-slate-700/10 placeholder:text-transparent" placeholder="name" value="<?= $curr->getName() ?>" />
            <label for="currname" class="absolute left-0 ml-2 px-1 -translate-y-3 bg-white text-sm duration-100 ease-linear peer-placeholder-shown:translate-y-3 peer-placeholder-shown:text-base peer-placeholder-shown:text-gray-500 peer-focus:ml-2 peer-focus:-translate-y-3 peer-focus:px-1 peer-focus:text-sm">Curriculum Name <span class="text-danger">*</span></label>
        </div>
        <div class="relative mb-6">
            <select name="course" id="course" class="px-4 py-3 peer w-full border focus:outline-none focus:ring-2 focus:border-brand focus:ring-brand/20 rounded-md border-slate-700/10 placeholder:text-transparent">
                ...
                ?>
                    <option value="<?= $course->getId() ?>" <?= $course->getId() == $curr->getCourse_id() ? "selected" : "" ?>><?= $course->getCourse() ?></option>
                <?php
                endforeach;
                ?>
            </select>
        ...
        <div class="relative mb-6">
            <select name="sy" id="sy" class="px-4 py-3 peer w-full border focus:outline-none focus:ring-2 focus:border-brand focus:ring-brand/20 rounded-md border-slate-700/10 placeholder:text-transparent">
                ...
                ?>
                    <option value="<?= $sy->getId() ?>" <?= $sy->getId() == $curr->getSy() ? "selected" : "" ?>><?= $sy->getSchoolyear() ?></option>
                <?php
                endforeach;
                ?>
            </select>
